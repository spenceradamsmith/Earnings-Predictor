...
async function fetchFullStockData(ticker) {
  const res  = await fetch(
    `https://flask-api-prod.us-east-1.elasticbeanstalk.com/predict?ticker=${encodeURIComponent(ticker)}`
  );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchAndDisplay(category) {
    ...
    const responses = await Promise.all(tickers.map(async ticker => {
      try {
        const res  = await fetch(`https://flask-api-prod.us-east-1.elasticbeanstalk.com/predict?ticker=${encodeURIComponent(ticker)}`);
